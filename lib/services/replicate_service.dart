...
class ReplicateService {
  ...
  static const String _versionId =
      'da77bc59ee60423279fd632efb4795ab731d9e3ca9705ef3341091fb989b7eaf';

  static const Map<String, String> _mockHairstyleImages = {
    'Crew Cut':
        'https://images.unsplash.com/photo-1590540179852-21102545e23a?q=80&w=800&auto=format&fit=crop',
    'Buzz Cut':
        'https://images.unsplash.com/photo-1593351915096-38f0d852abc0?q=80&w=800&auto=format&fit=crop',
    'Sports Buzz':
    ...
    'Messy Hair':
        'https://images.unsplash.com/photo-1503443207922-dff7d543fd0e?q=80&w=800&auto=format&fit=crop',
    'Korean Middle Part':
        'https://images.unsplash.com/photo-1519345182560-3f2917c472ef?q=80&w=800&auto=format&fit=crop',
    'Mohawk':
        'https://images.unsplash.com/photo-1501196351401-f74cf9915d05?q=80&w=800&auto=format&fit=crop',
    'Curly Hair':
  ...
  Future<String> generateHairstyleWithReplicate({
    ...
    try {
      debugPrint(
        '🚀 [Replicate] Creating prediction using version endpoint...',
      );
      ...
      final prompt =
          "Apply hairstyle $hairstyleName. Keep same identity, face, lighting, pose. Only change hair.";
      ...
      if (response.statusCode == 404) {
        debugPrint(
          '❌ [Replicate] HTTP 404: Resource not found. Check endpoint and version: ${response.body}',
        );
        throw Exception(
          'Replicate resource not found (404). Check API endpoint and version.',
        );
      }

      if (response.statusCode == 422) {
        debugPrint(
          '❌ [Replicate] HTTP 422: Invalid input or model access issue: ${response.body}',
        );
      ...
      if (response.statusCode == 401 || response.statusCode == 403) {
        debugPrint(
          '❌ [Replicate] Auth error (${response.statusCode}): ${response.body}',
        );
        throw Exception(
          'Replicate authentication/permission error: ${response.body}',
        );
      }

      if (response.statusCode != 201) {
        debugPrint(
          '❌ [Replicate] Failed to create prediction (${response.statusCode}): ${response.body}',
        );
      ...
      debugPrint(
        '📡 [Replicate] Prediction created (ID: $predictionId). Polling for status...',
      );
  ...
  Future<String> _generateMockHairstyle(String hairstyleName) async {
    ...
    final mockUrl =
        _mockHairstyleImages[hairstyleName] ??
